...
def bernoulli_log_likelihood(data, theta):
    ...
    return float(np.sum(data * np.log(theta) + (1 - data) * np.log(1 - theta)))
...
def bernoulli_mle_with_comparison(data, candidate_thetas=None):
    ...
    best_candidate = max(log_likelihoods, key=lambda t: log_likelihoods[t])
...
def poisson_log_likelihood(data, lam):
    ...
    return float(sum(x * np.log(lam) - lam - math.lgamma(x + 1) for x in data))
...
def poisson_mle_analysis(data, candidate_lambdas=None):
    ...
    best_candidate = max(log_likelihoods, key=lambda l: log_likelihoods[l])
